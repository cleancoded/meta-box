 ...
 * Map field class.
 */
class cleancoded_Map_Field extends cleancoded_Field {
	/**
	 * Enqueue scripts and styles.
	 */
	public static function admin_enqueue_scripts() {
		wp_enqueue_style( 'cleancoded-map', cleancoded_CSS_URL . 'map.css', array(), cleancoded_VER );
		 ...
		 */
		$google_maps_url = apply_filters( 'cleancoded_google_maps_url', $google_maps_url );
		wp_register_script( 'google-maps', esc_url_raw( $google_maps_url ), array(), cleancoded_VER, true );
		wp_enqueue_script(
			'cleancoded-map',
			cleancoded_JS_URL . 'map.js',
			array(
	...
	public static function html( $meta, $field ) {
		...
		$html    = sprintf(
			'<div class="cleancoded-map-field" data-address-field="%s">',
			esc_attr( $address )
		);

		$html .= sprintf(
			'<div class="cleancoded-map-canvas" data-default-loc="%s" data-region="%s"></div>
			<input type="hidden" name="%s" class="cleancoded-map-coordinate" value="%s">',
			esc_attr( $field['std'] ),
	...
	public static function render_map( $location, $args = array() ) {
		...
		$google_maps_url = apply_filters( 'cleancoded_google_maps_url', $google_maps_url );
		wp_register_script( 'google-maps', esc_url_raw( $google_maps_url ), array(), cleancoded_VER, true );
		wp_enqueue_script( 'cleancoded-map-frontend', cleancoded_JS_URL . 'map-frontend.js', array( 'google-maps', 'jquery' ), cleancoded_VER, true );
		...
		$output = sprintf(
			'<div class="cleancoded-map-canvas" data-map_options="%s" style="width:%s;height:%s"></div>',
