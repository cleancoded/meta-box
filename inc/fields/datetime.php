 ...
 */
class cleancoded_Datetime_Field extends cleancoded_Text_Field {
	/**
	...
	public static function admin_register_scripts() {
		...
		wp_register_style( 'cleancoded-date', cleancoded_CSS_URL . 'date.css', array( 'jquery-ui-datepicker' ), '1.8.17' );

		wp_register_style( 'jquery-ui-slider', "$url/jquery.ui.slider.css", array( 'jquery-ui-core', 'jquery-ui-theme' ), '1.8.17' );
		wp_register_style( 'jquery-ui-timepicker', "$url/jquery-ui-timepicker-addon.min.css", array( 'cleancoded-date', 'jquery-ui-slider' ), '1.5.0' );

		$url = cleancoded_JS_URL . 'jqueryui';
		...
		wp_register_script( 'cleancoded-datetime', cleancoded_JS_URL . 'datetime.js', array( 'jquery-ui-datepicker', 'jquery-ui-timepicker-i18n', 'underscore' ), cleancoded_VER, true );
		wp_register_script( 'cleancoded-date', cleancoded_JS_URL . 'date.js', array( 'jquery-ui-datepicker', 'underscore' ), cleancoded_VER, true );
		wp_register_script( 'cleancoded-time', cleancoded_JS_URL . 'time.js', array( 'jquery-ui-timepicker-i18n' ), cleancoded_VER, true );
		...
		foreach ( $handles as $handle ) {
			cleancoded_Helpers_Field::localize_script_once( "cleancoded-$handle", 'cleancoded_' . ucfirst( $handle ), $data );
		}
	}
	...
	public static function admin_enqueue_scripts() {
		...
		wp_enqueue_script( 'cleancoded-datetime' );
	}
	...
	public static function html( $meta, $field ) {
		...
		if ( $field['timestamp'] ) {
			...
			$output .= sprintf(
				'<input type="hidden" name="%s" class="cleancoded-datetime-timestamp" value="%s">',
		...
		if ( $field['inline'] ) {
			$output .= '<div class="cleancoded-datetime-inline"></div>';
		}
	...
	public static function meta( $post_id, $saved, $field ) {
		...
		if ( $field['timestamp'] ) {
			return cleancoded_Helpers_Array::map( $meta, __CLASS__ . '::from_timestamp', $field );
		}

		if ( $field['save_format'] && $meta ) {
			return cleancoded_Helpers_Array::map( $meta, __CLASS__ . '::from_save_format', $field );
		}
