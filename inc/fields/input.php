 ...
 */
abstract class cleancoded_Input_Field extends cleancoded_Field {
	/**
	 * Enqueue scripts and styles.
	 */
	public static function admin_enqueue_scripts() {
		wp_enqueue_style( 'cleancoded-input', cleancoded_CSS_URL . 'input.css', '', cleancoded_VER );
	}
	...
	public static function html( $meta, $field ) {
		...
		$wrapper_class = 'cleancoded-input-group';
		if ( version_compare( get_bloginfo( 'version' ), '5.3', '>=' ) ) {
			$wrapper_class .= ' cleancoded-input-dark';
		}
		...
		if ( $field['prepend'] ) {
			$output .= '<span class="cleancoded-input-group-prepend">' . esc_html( $field['prepend'] ) . '</span>';
		}
		...
		if ( $field['append'] ) {
			$output .= '<span class="cleancoded-input-group-append">' . esc_html( $field['append'] ) . '</span>';
		}
