 ...
 */
class cleancoded_OSM_Field extends cleancoded_Field {
	/**
	...
	public static function admin_enqueue_scripts() {
		...
		wp_enqueue_style( 'cleancoded-osm', cleancoded_CSS_URL . 'osm.css', array( 'leaflet' ), cleancoded_VER );
		wp_enqueue_script( 'cleancoded-osm', cleancoded_JS_URL . 'osm.js', array( 'jquery', 'jquery-ui-autocomplete', 'leaflet' ), cleancoded_VER, true );
	...
	public static function html( $meta, $field ) {
		...
		$html    = sprintf(
			'<div class="cleancoded-osm-field" data-address-field="%s">',
			esc_attr( $address )
		);

		$html .= sprintf(
			'<div class="cleancoded-osm-canvas" data-default-loc="%s" data-region="%s" data-language="%s"></div>
			<input type="hidden" name="%s" class="cleancoded-osm-coordinate" value="%s">',
			esc_attr( $field['std'] ),
	...
	public static function render_map( $location, $args = array() ) {
		...
		wp_enqueue_style( 'leaflet', 'https://unpkg.com/leaflet@1.5.1/dist/leaflet.css', array(), '1.5.1' );
		wp_enqueue_script( 'leaflet', 'https://unpkg.com/leaflet@1.5.1/dist/leaflet.js', array(), '1.5.1', true );
		wp_enqueue_script( 'cleancoded-osm-frontend', cleancoded_JS_URL . 'osm-frontend.js', array( 'jquery', 'leaflet' ), cleancoded_VER, true );
		...
		$output = sprintf(
			'<div class="cleancoded-osm-canvas" data-osm_options="%s" style="width:%s;height:%s"></div>',
