...
class RW_Meta_Box {
	...
	public function __construct( $meta_box ) {
		...
		$this->meta_box = apply_filters( 'cleancoded_meta_box_settings', $this->meta_box );
	...
	public function register_fields() {
		$field_registry = cleancoded_get_registry( 'field' );
	...
	public function is_shown() {
		$show = apply_filters( 'cleancoded_show', true, $this->meta_box );

		return apply_filters( "cleancoded_show_{$this->id}", $show, $this->meta_box );
	}
	...
	protected function global_hooks() {
		...
		foreach ( $this->fields as $field ) {
			cleancoded_Field::call( $field, 'add_actions' );
		}
	}
	...
	public function enqueue() {
		...
		wp_enqueue_style( 'cleancoded', cleancoded_CSS_URL . 'style.css', array(), cleancoded_VER );
		if ( is_rtl() ) {
			wp_enqueue_style( 'cleancoded-rtl', cleancoded_CSS_URL . 'style-rtl.css', array(), cleancoded_VER );
		}

		wp_enqueue_script( 'cleancoded', cleancoded_JS_URL . 'script.js', array( 'jquery' ), cleancoded_VER, true );

		// Load clone script conditionally.
		foreach ( $this->fields as $field ) {
			if ( $field['clone'] ) {
				wp_enqueue_script( 'cleancoded-clone', cleancoded_JS_URL . 'clone.js', array( 'jquery-ui-sortable' ), cleancoded_VER, true );
				break;
			}
		}
		...
		foreach ( $this->fields as $field ) {
			cleancoded_Field::call( $field, 'admin_enqueue_scripts' );
		}

		// Auto save.
		if ( $this->autosave ) {
			wp_enqueue_script( 'cleancoded-autosave', cleancoded_JS_URL . 'autosave.js', array( 'jquery' ), cleancoded_VER, true );
		}
		 ...
		 */
		do_action( 'cleancoded_enqueue_scripts', $this );
	}
	...
	public function show() {
		...
		printf(
			...
			esc_attr( trim( "cleancoded-meta-box {$this->class}" ) ),
			esc_attr( $this->autosave ? 'true' : 'false' ),
			esc_attr( $this->object_type ),
			esc_attr( $this->object_id )
		);

		wp_nonce_field( "cleancoded-save-{$this->id}", "nonce_{$this->id}" );

		// Allow users to add custom code before meta box content.
		// 1st action applies to all meta boxes.
		// 2nd action applies to only current meta box.
		do_action( 'cleancoded_before', $this );
		do_action( "cleancoded_before_{$this->id}", $this );

		foreach ( $this->fields as $field ) {
			cleancoded_Field::call( 'show', $field, $saved, $this->object_id );
		}

		// Allow users to add custom code after meta box content.
		// 1st action applies to all meta boxes.
		// 2nd action applies to only current meta box.
		do_action( 'cleancoded_after', $this );
		do_action( "cleancoded_after_{$this->id}", $this );
	...
	public function save_post( $object_id ) {
		...
		// Before save action.
		do_action( 'cleancoded_before_save_post', $object_id );
		do_action( "cleancoded_{$this->id}_before_save_post", $object_id );

		array_map( array( $this, 'save_field' ), $this->fields );

		// After save action.
		do_action( 'cleancoded_after_save_post', $object_id );
		do_action( "cleancoded_{$this->id}_after_save_post", $object_id );
	}
	...
	public function save_field( $field ) {
		...
		$old     = cleancoded_Field::call( $field, 'raw_meta', $this->object_id );
		$new     = cleancoded_request()->post( $field['id'], $default );
		$new     = cleancoded_Field::process_value( $new, $this->object_id, $field );

		// Filter to allow the field to be modified.
		$field = cleancoded_Field::filter( 'field', $field, $field, $new, $old );

		// Call defined method to save meta value, if there's no methods, call common one.
		cleancoded_Field::call( $field, 'save', $new, $old, $this->object_id );

		cleancoded_Field::filter( 'after_save_field', null, $field, $new, $old, $this->object_id );
	}
	...
	public function validate() {
		$nonce = cleancoded_request()->filter_post( "nonce_{$this->id}", FILTER_SANITIZE_STRING );

		return ! $this->saved
			&& ( ! defined( 'DOING_AUTOSAVE' ) || $this->autosave )
			&& wp_verify_nonce( $nonce, "cleancoded-save-{$this->id}" );
	}
	...
	public static function normalize( $meta_box ) {
		 ...
		 * @since 4.4.1
		 */
		cleancoded_Helpers_Array::change_key( $meta_box, 'pages', 'post_types' );

		// Make sure the post type is an array and is sanitized.
		$meta_box['post_types'] = array_map( 'sanitize_key', cleancoded_Helpers_Array::from_csv( $meta_box['post_types'] ) );
	 ...
	 * @param cleancoded_Storage_Interface $storage Storage object. Optional.
	 *
	...
	public static function normalize_fields( $fields, $storage = null ) {
		foreach ( $fields as $k => $field ) {
			...
			$field = apply_filters( 'cleancoded_normalize_field', $field );
			$field = apply_filters( "cleancoded_normalize_{$field['type']}_field", $field );
			$field = apply_filters( "cleancoded_normalize_{$field['id']}_field", $field );
	...
	public function is_saved() {
		foreach ( $this->fields as $field ) {
			...
			$value = cleancoded_Field::call( $field, 'raw_meta', $this->object_id );
			if ( false === $value ) {
				continue;
			}
			if (
	...
	public function get_storage() {
		return cleancoded_get_storage( $this->object_type, $this );
	}
