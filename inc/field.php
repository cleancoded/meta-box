 ...
 * The field base class.
 */
abstract class cleancoded_Field {
	/**
	 * Add actions.
	 */
	...
	public static function show( $field, $saved, $post_id = 0 ) {
		...
		if ( $field['clone'] ) {
			$field_html = cleancoded_Clone::html( $meta, $field );
		} else {
		...
		$classes = "cleancoded-field cleancoded-{$field['type']}-wrapper " . $field['class'];
	...
	public static function begin_html( $meta, $field ) {
		...
		if ( $field['name'] ) {
			$field_label = sprintf(
				'<div class="cleancoded-label">
					<label for="%s">%s%s</label>
					%s
				</div>',
				esc_attr( $field['id'] ),
				$field['name'],
				$field['required'] || ! empty( $field['attributes']['required'] ) ? '<span class="cleancoded-required">*</span>' : '',
	...
	public static function end_html( $meta, $field ) {
		return cleancoded_Clone::add_clone_button( $field ) . self::call( 'input_description', $field ) . '</div>';
	}
	 ...
	 * @param array $field     Field parameters.
	 * @param array $args      Arguments of {@see cleancoded_meta()} helper.
	 *
	 * @return mixed
	 */
	public static function raw_meta( $object_id, $field, $args = array() ) {
		...
		if ( isset( $field['storage'] ) ) {
			...
			$storage = cleancoded_get_storage( $args['object_type'] );
		} else {
			$storage = cleancoded_get_storage( 'post' );
		}
	...
	public static function meta( $post_id, $saved, $field ) {
		...
		if ( $field['clone'] ) {
			$meta = cleancoded_Helpers_Array::ensure( $meta );
			...
			if ( $field['multiple'] ) {
				...
				$meta = is_array( $first ) ? array_map( 'cleancoded_Helpers_Array::ensure', $meta ) : array( $meta );
			}
	...
	public static function process_value( $value, $object_id, $field ) {
		...
		if ( $field['clone'] ) {
			$value = cleancoded_Clone::value( $value, $old_value, $object_id, $field );
		} else {
	...
	public static function save( $new, $old, $post_id, $field ) {
		...
		if ( ! cleancoded_Helpers_Value::is_valid_for_field( $new ) ) {
	...
	public static function get_attributes( $field, $value = null ) {
		...
		$attributes['class'] = trim( implode( ' ', array_merge( array( "cleancoded-{$field['type']}" ), (array) $attributes['class'] ) ) );
	...
	public static function render_attributes( $attributes ) {
		...
		$attributes = array_filter( $attributes, 'cleancoded_Helpers_Value::is_valid_for_attribute' );
	 ...
	 * cleancoded_the_field function later.
	 *
	 * @use self::get_value()
	 * @see cleancoded_the_value()
	 *
	 * @param  array    $field   Field parameters.
	 * @param  array    $args    Additional arguments. Rarely used. See specific fields for details.
	 * @param  int|null $post_id Post ID. null for current post. Optional.
	...
	public static function call() {
		...
		return call_user_func_array( array( cleancoded_Helpers_Field::get_class( $field ), $method ), $args );
	}
	 ...
	 * Filters:
	 * - cleancoded_{$name}
	 * - cleancoded_{$field['type']}_{$name}
	 * - cleancoded_{$field['id']}_{$name}
	 *
	 * @return mixed
	 */
	public static function filter() {
		...
		if ( $field['id'] ) {
			...
			$filters[] = 'cleancoded_' . $field_id . '_' . $name;
		}
