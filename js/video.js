( function ( $, cleancoded ) {
		...
		createList: function ()
		{
			this.list = new MediaList( {
				...
				itemView: MediaItem.extend( {
					className: 'cleancoded-video-item',
					template : wp.template( 'cleancoded-video-item' ),
					render: function()
					{
	...
	function init( e ) {
		$( e.target ).find( '.cleancoded-video' ).each( initVideoField );
	}

	cleancoded.$document
		.on( 'mb_ready', init )
		.on( 'clone', '.cleancoded-video', initVideoField );
} )( jQuery, cleancoded );
