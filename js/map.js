( function ( $, document, window, google, cleancoded, i18n ) {
	'use strict';
	...
	MapField.prototype = {
		...
		initDomElements: function () {
			this.$canvas = this.$container.find( '.cleancoded-map-canvas' );
			this.canvas = this.$canvas[0];
			this.$coordinate = this.$container.find( '.cleancoded-map-coordinate' );
		...
		addListeners: function () {
			...
			$( window ).on( 'cleancoded_map_refresh', refresh );

			// Refresh on meta box hide and show
			cleancoded.$document.on( 'postbox-toggled', refresh );
		...
		autocomplete: function () {
			...
			if ( $( '.cleancoded-geo-binding' ).length ) {
				var geocodeAddress = that.geocodeAddress.bind( that );
		...
		findAddressField: function( fieldName ) {
			...
			$address = this.$container.closest( '.cleancoded-group-clone' ).find( 'input[name*="[' + fieldName + ']"]' );
			if ( $address.length ) {
				return $address;
			}

			// If map and address is inside a non-cloneable group.
			$address = this.$container.closest( '.cleancoded-group-wrapper' ).find( 'input[name*="[' + fieldName + ']"]' );
			if ( $address.length ) {
				return $address;
			}
	...
	function init( e ) {
		$( e.target ).find( '.cleancoded-map-field' ).each( createController );
	}

	function restart() {
		$( '.cleancoded-map-field' ).each( createController );
	}

	cleancoded.$document
		.on( 'mb_ready', init )
		.on( 'clone', '.cleancoded-input', restart );
} )( jQuery, document, window, google, cleancoded, cleancoded_Map );
