( function ( $, wp, window, cleancoded ) {
	...
	function getOriginalId( $el ) {
		var $clone = $el.closest( '.cleancoded-clone' ),
			currentId = $clone.find( '.cleancoded-wysiwyg' ).attr( 'id' );
	...
	function init( e ) {
		$( e.target ).find( '.cleancoded-wysiwyg' ).each( transform );
	}
	...
	cleancoded.$document
		...
		.on( 'clone', '.cleancoded-wysiwyg', function() {
			/*
