( function ( $, _, cleancoded, i18n ) {
	...
	function transform() {
		var $this = $( this ),
			...
			$inline = $this.siblings( '.cleancoded-datetime-inline' ),
			$timestamp = $this.siblings( '.cleancoded-datetime-timestamp' ),
			current = $this.val(),
	...
	function init( e ) {
		$( e.target ).find( '.cleancoded-datetime' ).each( transform );
	}

	setTimeI18n();
	cleancoded.$document
		.on( 'mb_ready', init )
		.on( 'clone', '.cleancoded-datetime', transform );
} )( jQuery, _, cleancoded, cleancoded_Datetime );
