( function( $, L, cleancoded, i18n ) {
	...
	OsmField.prototype = {
		...
		initDomElements: function () {
			this.$canvas = this.$container.find( '.cleancoded-osm-canvas' );
			this.canvas = this.$canvas[0];
			this.$coordinate = this.$container.find( '.cleancoded-osm-coordinate' );
	...
	function init( e ) {
		$( e.target ).find( '.cleancoded-osm-field' ).each( createController );
	}

	function restart() {
		$( '.cleancoded-osm-field' ).each( createController );
	}

	cleancoded.$document
		.on( 'mb_ready', init )
		.on( 'clone', '.cleancoded-input', restart );
} )( jQuery, L, cleancoded, cleancoded_Osm );