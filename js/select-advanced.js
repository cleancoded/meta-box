( function ( $, cleancoded ) {
	...
	function transform() {
		...
		if ( options.ajax_data ) {
			...
			options.ajax.transport = function ( params, success, failure ) {
				...
				var actions = {
					'post'             : 'cleancoded_get_posts',
					'taxonomy'         : 'cleancoded_get_terms',
					'taxonomy_advanced': 'cleancoded_get_terms',
					'user'             : 'cleancoded_get_users'
				};
	...
	function init( e ) {
		$( e.target ).find( '.cleancoded-select_advanced' ).each( transform );
	}

	cleancoded.$document
		.on( 'mb_ready', init )
		.on( 'clone', '.cleancoded-select_advanced', transform );
} )( jQuery, cleancoded );
