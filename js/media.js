( function ( $, wp, _, cleancoded, i18n ) {
	'use strict';

	var views = cleancoded.views = cleancoded.views || {},
		models = cleancoded.models = cleancoded.models || {},
		media = wp.media,
	...
	MediaCollection = Backbone.Collection.extend( {
		...
		remove: function ( models, options ) {
			...
			if( ! $( event.target ).closest( '.cleancoded-field, [data-class="cleancoded-field"]' ).length ) {
				return;
			}
	...
	MediaField = views.MediaField = Backbone.View.extend( {
		className: 'cleancoded-media-view',
		initialize: function ( options ) {
			...
			this.controller.get( 'items' ).on( 'add remove reset', _.debounce( function () {
				that.$input.trigger( 'change', [that.$( '.cleancoded-media-input' )] );
			}, 500 ) );
	...
	MediaList = views.MediaList = Backbone.View.extend( {
		tagName: 'ul',
		className: 'cleancoded-media-list',
	...
	MediaStatus = views.MediaStatus = Backbone.View.extend( {
		tagName: 'div',
		className: 'cleancoded-media-status',
		template: wp.template( 'cleancoded-media-status' ),
	...
	MediaButton = views.MediaButton = Backbone.View.extend( {
		tagName: 'div',
		className: 'cleancoded-media-add',
		template: wp.template( 'cleancoded-media-button' ),
		events: {
	...
	MediaItem = views.MediaItem = Backbone.View.extend( {
		tagName: 'li',
		className: 'cleancoded-media-item attachment',
		template: wp.template( 'cleancoded-media-item' ),
		initialize: function ( options ) {
			this.controller = options.controller;
			this.collection = this.controller.get( 'items' );
			this.render();
		...
		events: {
			'click .cleancoded-image-overlay': function ( e ) {
				e.preventDefault();
				this.trigger( 'click:switch', this.model );
			},
			'click .cleancoded-remove-media': function ( e ) {
				e.preventDefault();
				this.trigger( 'click:remove', this.model );
			},
			'click .cleancoded-edit-media': function ( e ) {
				e.preventDefault();
	...
	MediaSelect = views.MediaSelect = MediaFrame.Select.extend( {
		...
		createStates: function () {
			...
			this.$el.attr( 'data-class', 'cleancoded-field' );
	...
	function init( e ) {
		$( e.target ).find( '.cleancoded-file_advanced' ).each( initMediaField );
	}

	cleancoded.$document
		.on( 'mb_ready', init )
		.on( 'clone', '.cleancoded-file_advanced', initMediaField );
} )( jQuery, wp, _, cleancoded, i18ncleancodedMedia );
